...
class Space(object):
    ...
    def move_player(self, player, board):
        ...
        if self.move > 0:
            print("This space has a ladder.\n"+player.piece+"moves up",self.move,"spaces")
        elif self.move < 0:
            print("This space has a chute.\n" + player.piece + "moves down", self.move, "spaces")
...
def ask_num(question):
    response = None
    while response not in range(2,5):
...
def main():
    num_players = ask_num("how many players will be playing")
    players = []
    turn = 0
    winner = None
